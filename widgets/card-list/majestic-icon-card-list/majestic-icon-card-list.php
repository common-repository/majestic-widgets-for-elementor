...
class Majestic_icon_card_list extends \Elementor\Widget_Base
{
    ...
    {
        return 'majestic-icon-card-list';
    }
...
$this->add_control(
    ...
    [
        ...
        $this->add_control(
            ...
            [
                'label' => 'Sub Description Alignment',
                'type' => \Elementor\Controls_Manager::SELECT,
                'default' => 'left',
                'options' => [
                    'left' => 'Left',
            ...
            [
                ...
                'fields' => [
                    ...
                    [
                        ...
                        'default' => 'fa fa-star', // Set a default icon class
                    ],
                    [
                        'name' => 'icon_size',
                        'label' => 'Icon Size (px)',
                        ...
                        'default' => [
                            'unit' => 'px',
                            'size' => 24, // Set the default icon size
                        ],
                    ],
                    [
                        'name' => 'icon_color',
                        'label' => 'Icon Color',
                        'type' => \Elementor\Controls_Manager::COLOR,
                        'default' => '#333', // Set the default icon color
                    ],
                ...
                'default' => [
                    [
                        'card_icon' => 'fa fa-star', // Default icon class
    ...
    protected function render()
    {
        ...
        if (!empty($settings['cards'])) {
            echo '<div class="majestic-icon-card-list">';
 ...
 $flex_direction = $settings['flex_direction'];
            echo '<div class="cards ' . $flex_direction . '"';
                // ... Previous code ...
                ...
                if (!empty($card['card_icon'])) {
                    echo '<div class="card-icon" style="float: left; margin-right: 10px;"><i class="' . $card['card_icon'] . '" style="font-size: ' . $card['icon_size']['size'] . $card['icon_size']['unit'] . '; color: ' . $card['icon_color'] . ';"></i></div>';
                }
        
                // Content on the right
                echo '<div class="card-content" style="float: left;">';
        

                if ($settings['hide_title'] !== 'yes') {
                    echo '<h2 style="text-align: ' . $settings['title_alignment'] . ';font-size: ' . $font_size . '; font-family: ' . $font_family . ', sans-serif; color: ' . $text_color . ';">' . $card['card_title'] . '</h2>';
                }
                
                if ($settings['hide_description'] !== 'yes') {
                    echo '<p style="text-align: ' . $settings['description_alignment'] . ';font-size: ' . $description_font_size . '; font-family: ' . $description_font_family . ', sans-serif; color: ' . $description_text_color . '; text-align: ' . $settings['description_alignment'] . ';">' . $card['card_description'] . '</p>';
                }
                
                if ($settings['hide_sub_description'] !== 'yes') {
                    echo '<p class="sub-description" style="text-align: ' . $settings['sub_description_alignment'] . ';font-size: ' . $custom_description_font_size . '; font-family: ' . $custom_description_font_family . ', sans-serif; color: ' . $custom_description_text_color . '; text-align: ' . $settings['sub_description_alignment'] . ';">' . $card['card_subdescription'] . '</p>';
                }
...
// Register the widget
\Elementor\Plugin::instance()->widgets_manager->register_widget_type(new Majestic_icon_card_list());
function majestic_icon_card_list_register_widget_styles()
{
    wp_enqueue_style(
        'majestic-icon-card-list-style',
        plugin_dir_url(__FILE__) . '/css/majestic-icon-card-list.css'
    );
}
add_action('elementor/frontend/before_enqueue_scripts', 'majestic_icon_card_list_register_widget_styles');

function majestic_icon_card_list_register_widget_scripts()
{
    wp_enqueue_script(
        'majestic-icon-card-list-script',
        plugin_dir_url(__FILE__) . '/js/majestic-icon-card-list.js', // Replace with your script file path
        ['jquery'], // You can specify jQuery as a dependency
        '1.0.0', // Replace with your script version
        true
    );
}
add_action('elementor/frontend/before_enqueue_scripts', 'majestic_icon_card_list_register_widget_scripts');

function majestic_icon_card_list_register_widget_styles_icon()
{
    // Enqueue Font Awesome CSS
    wp_enqueue_style('font-awesome', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css');

    // Enqueue your widget's custom styles
    wp_enqueue_style(
        'majestic-icon-card-list-style',
        plugin_dir_url(__FILE__) . '/css/majestic-icon-card-list.css'
    );
}
add_action('elementor/frontend/before_enqueue_scripts', 'majestic_icon_card_list_register_widget_styles_icon');
